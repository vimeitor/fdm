...
public class Algorithm {
    ...
    Double[] snell(int n, Double[] space) { // parametros: numero de regiones del espacio y
        // vector de indices de refraccion
        ...
        int reg;
        Double tiempo0 = calculo_tiempo_luz(n, space, sep_coord); // tiempo que tarad la luz con
        // las coordenadas actuales
        int cont = 0;
        while (!stop) {
            reg = ThreadLocalRandom.current().nextInt(1, n - 1);
            alt = ThreadLocalRandom.current().nextDouble(-d, d);
            ; //coge unas nuevas coordenadas para un punto de interseccion aleatorio.
            new_sep_coord = sep_coord;
            new_sep_coord[reg] = sep_coord[reg] + alt;
            nuevotiempo = calculo_tiempo_luz(n, space, new_sep_coord);  //comprueba si con las
            // nuevas coordenadas el tiempo es menor)
    ...
    Double calculo_tiempo_luz(int n, Double[] space, Double[] sep_coord) { //calculo del tiempo
        // que tarda la luz en
        Double velocidad_luz = 1.0;                                    // atravesar el espacio
        // con esa direccion
    ...
    Double[] snellfijado(int n, Double[] space) { // parametros: numero de regiones del espacio y
        // vector de indices de refraccion
        Double[] sep_coord = new Double[n + 1]; // vector de coordenadas y(i)
        Double[] new_sep_coord = new Double[n + 1];
        boolean stop = false;
